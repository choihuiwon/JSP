	...
	<div id="main">
		...
		<div id="content">
			...
			<table class="board">
				...
				<tr>
					<td>${dto.bno }</td>
					<td><a href="boardView.do?bno=${dto.bno }">${dto.title } [${requestScope.dao.getBoardCommentCount(dto.bno) }]</a></td>
					<td>${dto.writer }</td>
					<td>${dto.bdate }</td>
					<td>${dto.bcount }</td>
					<td>${dto.blike }</td>
					...
					<td colspan="7">
						<div class="page_bar">
							...
							<c:if test="${sessionScope.login ne null && sessionScope.login eq true }">
								<a href="board/board_write_view.jsp" class="btn_writer">글쓰기</a>
							</c:if>
						</div>
					</td>
				</tr>
